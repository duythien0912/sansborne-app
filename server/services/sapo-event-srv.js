'use strict';
const { Event } = require('../dal/models');
...
class SapoEventsrv {
  ...
  static async upsertMany(body) {
    ...
    const { nModified } = await Event.bulkWrite(bulkUpdateOps);
  ...
  static async updateMany(body) {
    ...
    const { nModified } = await Event.updateMany(
  ...
  static async deleteMany(ids) {
    const { deletedCount } = await Event.deleteMany({ _id: { $in: ids } });
